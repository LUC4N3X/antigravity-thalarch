...
- **Runtime-state verdict precedence:** when the main proposition asks whether tests/build/lint/typecheck/benchmark/command execution currently passes or succeeds, first require a successful matching execution from the latest user request. Without that fresh successful run, the runtime proposition remains `UNKNOWN` or `UNVERIFIED`; source/config/static reasoning, a prior run, or a failed command cannot justify `PROVEN`/`SUPPORTED`. If a structured `unverified` ledger exists, name the missing runtime proof there.
- **Visual-state seal:** rendered appearance, visual fidelity, clipping, reference matching, and mobile/desktop layout claims require actual rendered pixels/browser/screenshot/device/vision evidence from the current request. Source, DOM, CSS, static reasoning, a generation prompt, or an old screenshot cannot prove current rendered appearance. If that proof was not observed, the main visual-state proposition must remain `UNKNOWN` or `UNVERIFIED`; do not use `CORRECTED_PREMISE`, `NOT_FOUND`, `PROVEN`, or `SUPPORTED` for the rendered claim. If a structured `unverified` ledger exists, record the missing render/browser/screenshot/viewport/mobile/desktop proof there.
